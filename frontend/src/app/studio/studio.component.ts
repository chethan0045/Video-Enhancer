      ...
      <aside class="props">
        ...
        <div class="psec" *ngIf="active==='subtitles'">
          ...
          <ng-container *ngIf="subs.enabled">
            ...
            <label class="chk"><input type="checkbox" [(ngModel)]="subs.metadata"/> Generate AI title & description</label>
        ...
        <div class="psec" *ngIf="active==='enhance'">
          ...
          <ng-container *ngIf="enhance.enabled">
            ...
            <label class="fld">Color<select [(ngModel)]="enhance.color"><option value="restore">✨ Auto Restore (faded video)</option><option value="">None</option><option value="cinematic">Cinematic</option><option value="teal_orange">Teal & Orange</option><option value="warm">Warm</option><option value="cool">Cool</option><option value="vintage">Vintage</option></select></label>
...
export class StudioComponent implements OnDestroy {
  ...
  subs = { enabled: false, language: 'auto', model: 'tiny', output: 'burn', translateTo: 'none', cleanup: false, metadata: false };
  speed = 1;
  rotate = 0;
  exportFormat = 'mp4';
  exportCodec = 'h264';
  ...
  async exportProject() {
    ...
    const pipeline = {
      ...
      subtitles: { enabled: this.subs.enabled, language: this.subs.language, model: this.subs.model, output: this.subs.output, translateTo: this.subs.translateTo, cleanup: this.subs.cleanup, metadata: this.subs.metadata },
